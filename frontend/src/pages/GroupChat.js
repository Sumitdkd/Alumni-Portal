...
export default function GroupChat() {
  ...
  useEffect(() => {
    const checkGroupMembership = async () => {
      try {
        ...
        if (!res.data.isMember) setNotMemberModalOpen(true);
      } catch (error) {
  ...
  //  Placeholder centered, text left when typing
  const mentionStyles = useMemo(() => {
    const empty = newMessage.trim().length === 0;
    return {
      control: {
        width: '100%',
        maxWidth: '100%',
        minHeight: '48px',
        padding: '12px 16px', // padding fixes cut-off issue
        borderRadius: '9999px',
  ...
  if (!isGroupMember && !loading) {
    return <NotMemberModal isOpen={isNotMemberModalOpen} onClose={CloseNotMemberModal} />;
  }
          ...
          messages.map((msg) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`p-2.5 sm:p-3 rounded-2xl max-w-[75%] sm:max-w-lg ${
                    isMyMessage
                      ? 'bg-blue-200 text-black rounded-br-none'
                      : 'bg-gray-200 text-on-surface rounded-bl-none'
                  }`}
                >
                  {!isMyMessage && (
